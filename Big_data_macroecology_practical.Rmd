...
#his questions are likely to be on the phylogeny of the tree situation
```
...
matA(comadre_sub)[[1]]
#so the columns are what they are this year, and the columns are what they are next year
...
#anstructural model - treating all individuals the same way - to calculate growth rate get size of pop now and size of it next year. no. individuals in future/no. individuals now gets the growth rate. if above 1 pop is growing, if 1 there's been no change in growth
#Making assumptions - closed system, also assuming all individuals are the same
#predictors of survival:
##age - convenient trait, can determine if can reproduce. Some species can estimate age, some can't - e.g. deer based on antler 
##sex
##size
#e.g. insect with well defined lifecycle - instar1, instar2, adult.
##instar 1 can't remain as instar 1 - instar2 or dead, get a 3 stage population model, get rates of survival and of development, reproduction
#F - fecundity - matrix to do with sexual reproduction
#c - clonality - happens a lot across the tree of life, if in a subset C is 0 it means working w a non clonally reproducing organism 
#U - all other survival that isn't based on sexual reproduction or cloning
#A is F, C and U mashed together
```
...
#i think this is a little bit dodgy because it is overwriting where r ur pipes bbgorl
```
